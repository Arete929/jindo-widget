...
//   위 줄(작게) = 창 이름 + 서비스, 아래 줄(크게·진하게) = 숫자. 폭은 두 줄 중 더 넓은 쪽만큼만
//   (대개 위 줄이 더 넓다) — 숫자만 있던 한 줄보다 가로 자리를 덜 먹는다.
//   무엇을 몇 줄·무슨 글자로 보여줄지는 main.js 의 usageTrayItems() 가 정한다 — 이 파일은
//   {key, lines:[[{t,c}...], [{t,c}...]], tip} 목록을 받아 그리고 트레이를 맞출 뿐, 어떤 서비스인지는 모른다.
//
// 그림을 어떻게 만드나
//   숨은 창 하나를 계속 재사용해 작은 HTML(줄마다 글자 여러 색)을 그리고, 전체 크기(getBoundingClientRect)를
...
/* lines = [ [{t:'5시간',c:MUTED},{t:'Claude',c:'#d97757'}], [{t:'20',c:'#3f6fd1'}] ] 처럼
   줄마다 이어 붙일 글자 조각들의 배열(줄 0=위=작게, 줄 1=아래=크게). 자리(하위 호환) 삼아 부른
   parts(평평한 배열)를 받아도 «한 줄» 로 그린다. 반환값은 트레이용 그림(NativeImage, 두 줄 높이) */
async function renderBadge(linesOrParts) {
  const w = getRenderWin();
  const lines = Array.isArray(linesOrParts[0]) ? linesOrParts : [linesOrParts];
  const rowsHtml = lines.map((parts, i) => {
    const fs = (i === lines.length - 1 ? FONT_NUM : FONT_LB) * SCALE;
    const spans = (parts || []).map((p) => '<span style="color:' + (p.c || MUTED) + '">' + esc(p.t) + '</span>').join(' ');
    return '<div class="ln" style="font-size:' + fs + 'px">' + spans + '</div>';
  }).join('');
  const html = 'data:text/html;charset=utf-8,' + encodeURIComponent(
    '<!doctype html><html><head><meta charset="utf-8"><style>'
    + 'html,body{margin:0;padding:0;background:transparent}'
    + '#col{display:inline-flex;flex-direction:column;align-items:center;'
    + 'font-family:"Segoe UI",Pretendard,sans-serif;font-weight:800;letter-spacing:-.3px;'
    + 'padding:' + SCALE + 'px}'
    + '.ln{white-space:nowrap;line-height:1.15;'
    /* 밝은·어두운 작업표시줄 어디서나 읽히게 — 반대색 옅은 그림자를 사방에 깐다 */
    + 'text-shadow:0 0 ' + SCALE + 'px rgba(255,255,255,.9), 0 0 ' + SCALE + 'px rgba(0,0,0,.55)}'
    + '</style></head><body><div id="col">' + rowsHtml + '</div></body></html>');
  await w.loadURL(html);
  const box = await w.webContents.executeJavaScript(
    '(function(){var r=document.getElementById("col").getBoundingClientRect();return {w:Math.ceil(r.width),h:Math.ceil(r.height)};})()');
...
/* items = [{ key, lines:[[{t,c}],[{t,c}]], tip, ...아무거나(눌렀을 때 onOpen 에 그대로 넘어감) }, …]
...
async function reconcile(items) {
  ...
  try {
    ...
    for (const it of list) {
      const sig = JSON.stringify(it.lines);
      let t = trays[it.key];
      if (!t || t.isDestroyed() || sigs[it.key] !== sig) {
        const img = await renderBadge(it.lines);
        if (!img) continue;
        if (!t || t.isDestroyed()) {
          t = new Tray(img);

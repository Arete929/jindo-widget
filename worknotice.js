// 파일명: worknotice.js | @version 1.115.1
// 수정요약: v1.115.1 부서 머리 끝의 마침표 허용(DEPT_RE·DEPT_CELL_RE) — «3. 생활상담부.» 가 부서로 안 잡혀 앞 부서
//   안쪽 글로 묻히고 보라 머리도 없던 것. PARSE_VER 3(저장된 옛 해석본을 버리고 다시 받음).
...
const DEPT_RE = /^\s*(\d{1,2})\s*[.．]\s*([가-힣0-9]{2,10}(?:부|실))\s*[.．]?\s*$/;   // 끝 마침표 허용(«3. 생활상담부.» 가 부서로 안 잡혀 보라 머리가 없던 것 — 2026-09-25)
...
const DEPT_CELL_RE = /^\s*(?:[·•]\s*|\d{1,2}\s*[.．]\s*)([가-힣0-9]{2,10}(?:부|실))\s*[.．]?\s*$/;
const ETC_RE = /^\s*기타\s*$/;

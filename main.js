...
function getFontScale() {
  ...
  const out = { work: 1, comci: 1, cal: 1, meal: 1, rec: 1, home: 1, note: 1, link: 1,
    grid: 1, office: 1, usage: 1, task: 1, tt: 1 };
...
ipcMain.on('set-ui', (_e, v) => {
  ...
  if (v.gradeWrite !== undefined && HAS_TT) {
    ...
    if (v.gradeWrite.url !== undefined) cur.url = String(v.gradeWrite.url || "").trim();
    if (v.gradeWrite.key !== undefined) cur.key = String(v.gradeWrite.key || "").trim();

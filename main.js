...
async function gradeBridge(몸) {
  ...
  for (let i = 0; i < 2; i++) {
    const r = await request({
      method: "POST", url: w.url, contentType: "application/json",
      body: JSON.stringify(보냄), timeout: 60000
    });
    let j = null;
  ...
  throw new Error("다리가 제대로 답하지 않았습니다 — " + 마지막);
}

...
const THEMES_JINHO = ['gold', 'chillwhite', 'hanok', 'night', 'darkui', 'slatelight', 'lavender'];
const THEMES_HYEWON = ['', 'black', 'slate', 'slatelight', 'persian', 'chillwhite', 'night',
  'hanok', 'darkui'];
function themeList() { return FLAVOR === 'jinho' ? THEMES_JINHO : THEMES_HYEWON; }
function getTheme() {
  let t = loadState().theme || '';
  if (t === 'ghost') t = '';        // 고스트가 곧 기본이다
  const 목록 = themeList();
  if (목록.includes(t)) return t;
  /* 목록에 없는 것이 저장돼 있으면(갈래가 바뀌었거나 예전 이름) 그 갈래의 첫 번째로 */
  return 목록[0];
}

// 파일명: main.js | @version 2.2.0
// 수정요약: v2.2.0 AI 사용량을 작업표시줄 알림 영역에 서비스마다 작은 숫자 아이콘으로(usagetray.js, 지비스 전용) —
//   Claude·Gemini·ChatGPT 를 켜 놓으면 트레이 대표 아이콘 옆에 %(로그인 필요는 !) 아이콘이 나란히 뜸, 눌러서 로그인·위젯 열기.
//   트레이 풍선 도움말(마우스 올리면)에도 켠 것의 %·남은 시간 붙임(usageTipLine) / v2.0.3 다리 요청마다 번호(req) — 결과가 학교 망에서 사라져 다시 보내도 다리(1.5.0)가 쓰기를 두 번 안 함 · 1·2.5·5초 네 번까지 / v2.0.2 다리 POST 가 구글 넘김 길에서 끊기면(404·«알 수 없는 api») 세 번까지 다시 보냄(bridgePost) — LG 혜원이지 출결 목록 안 뜨던 것 / v2.0.1 브라우저 인쇄 — 머리글·바닥글 없애기(@page 여백 0 + 종이 안 여백 thead·tfoot) · 배경색 늘 찍기(print-color-adjust) / v2.0.0 ★혜원이지 출결(연결 코드·나는 누구·역할별 반) + 지비스 관리자 15반 보기·동료 PC 는 OneDrive 설정 안 봄·틱틱 보는 목록 기억(tickView)·grZoom v-화면·주간업무 인쇄 표 가운데·이름 혜원이지로 되돌림 / v1.129.0 인쇄를 브라우저로 — 제 인쇄 창이 5초 안에 안 뜨면(학교 PC 출력물 보안) 임시 HTML 을 기본 브라우저로 열고 자동 print(), 그 PC 는 다음부터 곧장 브라우저 / v1.128.0 얹은 수업진도 웹앱 배율(dash-show zoom → setZoomFactor)·grZoom 열쇠 wid/easy(.progress|.comci|.dash) / v1.127.0 오늘 주간표 크기(grZoom, 창마다·이 PC) 저장 / v1.126.0 출결 «점검하기» 체크(서류·출석부·NEIS) — 앱 설정+OneDrive 에만(attChk), 시트엔 안 씀 / v1.125.1 인쇄 미리보기 단추 먹통 고침(paperload.js 빌드 목록 누락 + 다리 없을 때 기본 인쇄·닫기로) + 출결 저장 «먼저 보여 주기» / v1.125.0 인쇄 미리보기를 부른 창의 딸린 창으로(위젯 뒤로 안 숨음)·PDF 저장·실패 알림 + 출결 칸 너비 기억 / v1.124.0 테마 여섯·기본 슬레이트 라이트(고스트는 ghost 로 저장) + 인쇄 미리보기가 떠 있는 동안 위젯 «항상 위» 를 내려 맨 앞에 / v1.123.0 신청사유 내 예시 문구 저장(attPhrases, OneDrive 공유) / v1.122.0 출결 att-edit/att-del/att-local-edit / v1.121.1 미인정 출결을 앱에만(OneDrive 공유) + 종이 «맨몸» 모드(제목·앱 이름 없이) / v1.121.0 담임 출결 IPC(att-*, 다리 경유·혜비스 차단) + 설정 창에 일지 다리 값을 줘 빈 저장이 열쇠를 지우던 것 막음 / v1.113.0 학생기록 ↔ 노션 왕복(↗ 노션으로 · ↙ 가져오기) + 제목 «JI-VIS» 가 좁은 창에서 두 줄로 꺾이던 것 / v1.112.0 행발 변환기 보강 — 평서형 종결(했다·한다·왔다…)을 «~함» 으로, 학생 이름·1인칭 지우기, 혼잣말/물음 문장 버리기 / v1.111.1 저장해 둔 기록에서 «↻ 변환» 을 눌러도 칸이 비던 것 — 값을 넣자마자 다시 그리며 지워졌다(줄별로 담아 둔다) / v1.111.0 다른 세션의 «AI 사용량 초기화 시각 표시»(v1.110.0) 와 «#행특 복사» 를 한 판에 합침 / v1.110.0 학생기록에 «#행특 복사» — 노셔나이가 알아듣는 꼴로 클립보드에 담는다(열쇠·인터넷 없이). 노션에 붙여넣으면 [DB] 2026 학생기록에 페이지 생성+누가기록 문장 자동 작성 / v1.109.0 학생기록 — 마지막 학급·보기 기억 + 행발 누가기록 칸(시트 J열) 신설, «↻ 변환»으로 기록 내용을 생활기록부 문장으로 다듬음(노션 10.행동특성 기준), 보기(둘 다·기록만·누가기록만) / v1.108.1 업무관리를 굴릴 때 오른쪽 틱틱이 고정 머리 밑으로 파고들어 함께 올라가던 것 — 머리 아래에 붙이고 길면 제 안에서 굴러가게 / v1.108.0 ★이름 바꿈 — 진호알리미 → 지비스(JI-VIS). 제목 줄은 손글씨 Pacifico 로 «JI-VIS»(폰트 앱에 심음), 그 밖은 «지비스». 설치 이름·appId·설정 폴더는 그대로라 설정·로그인·자동 업데이트 모두 이어짐 / v1.107.1 진호 시간표가 새 배포를 즉각 반영 — 서비스워커·캐시 떼어 내고, 판이 바뀌면 스스로 다시 불러옴 + ⟳ 단추 / v1.107.0 진호 시간표(얹은 수업진도 웹앱)의 판을 서브칩 줄 오른쪽에 표시 — 배포본 js/server.js 에서 읽음(켤 때+6시간마다, 올리면 배포 시각) / v1.106.2 업무관리 칸막이가 620px 에서 막히던 것 — 창 끝까지 끌리게(왼쪽 280px 만 남김) + 넓게 잡은 값이 300 으로 되돌던 것 수정 / v1.106.1 AI 사용량 «타일 자체» 를 제목 줄 옆 여백으로 옮김(아래 띠 없앰) — 원형/막대·A−A+·⟳ 그대로, 초기화·결제는 올리면 뜨는 풀이로 / v1.106.0 AI 사용량을 제목 줄 «지비스» 옆 작은 숫자로 옮기고, 톱니 옆 ◍ 스위치로 큰 상자를 여닫음(지비스만·기억) / v1.105.2 넓게보기 폭 상한을 전부 풂 — 주간업무·학사일정도 창을 다 쓴다 / v1.105.1 넓게보기가 창을 다 쓰게 — #view max-width:1100px 을 풀어 오른쪽에 늘 남던 빈 자리 없앰(주간업무·학사일정만 글줄 상한 1250) / v1.105.0 AI 사용량 확대·축소(A− A+) — 글자·원형·막대가 함께 커지고 마지막 크기 기억. fontScale 저장 목록에 usage·task·tt 가 빠져 있던 것도 고침 / v1.104.0 틱틱 팔레트를 키보드로 — !높/^혜 처럼 이어 치면 걸러지고 Enter 로 고름, ↑↓ 옮기기, Esc 닫기 / v1.103.0 틱틱 입력칸에서 !·^ 를 치는 순간 우선순위·보관함 단추가 뜸(고르면 글자 지워지고 칩, Esc 닫기) — 칩 줄 위임 배선 / v1.102.1 틱틱 줄의 날짜·우선순위·보관함 칩이 밋밋한 글자로 보이던 것(.tdb 규칙이 덮음) — 버튼처럼 테두리·손가락 커서, 우선순위 없음은 «!없음» / v1.102.0 틱틱 완료함 — 위젯·틱틱 앱에서 완료한 것을 장부에 적어 접힌 «완료 N» 으로 보이고 복원·하나 지우기·전부 지우기(두 번) / v1.101.1 틱틱 마감일이 하루 이르게 보이던 것(UTC 문자열 자르기 → 현지 날짜) / v1.101.0 틱틱 손질 — 줄마다 날짜말·보관함·우선순위, 단축키(!높음 ^보관함 내일), 날짜·우선순위·보관함 팔레트(달력 포함), 지우기(두 번), 정렬, 목록 고르면 전체로 자동 전환 / v1.100.2 결제 크레딧 잔액이 늦게 그려져 비어 오던 것(잔액까지 기다림) / v1.100.1 틱틱 기본함을 /project/inbox/data 로 곧장 읽음(만들었다 지우는 우회 제거 — 그 길은 Unknown exception) / v1.100.0 클로드 결제 정보(플랜·다음 결제/취소 예정일 D-day·크레딧 잔액) 하루 1회 읽어 사용량 아래 표시 + 틱틱 기본함(Inbox) 목록에 포함 + 혜비스 동시 배포 / v1.99.0 내 바로가기 «담기»→«저장», 저장하면 곧바로 타일로 보이고 저장시각(KST) 표시·기억 / v1.98.0 런처보드 머리줄에 «＋ 담기»(앱 고치기 없이 한 번에) + 없는 단추를 가리키던 안내문 정리 / v1.97.1 런처보드 담기가 한 번 미끄러지면 조용히 잠긴 채 남던 문제(잠금 항상 해제·40초 시한·까닭 표시) / v1.97.0 학년부 일지 세부사항(E열)을 내용(D열) 아래에 줄바꿈해 바로 보여줌(누르지 않아도) / v1.96.0 학사일정 학년 스위치가 새로고침에 꺼지던 문제(자료 다시 받는 동안 도구줄이 통째로 사라짐) + 마지막으로 켠 학년 기억 / v1.95.2 얹은 수업진도 로그인을 관문이 걷힐 때까지 지켜보며 다시 잇는다(늦게 뜨면 놓치던 것) + 못 이을 땐 헛단추 대신 까닭 안내 / v1.95.1 얹은 수업진도가 탭 줄을 덮던 문제(화면 배율을 자리 계산에 안 넣었다) + 웹앱 제 머리를 감추고 바탕색을 테마에 맞춤 / v1.95.0 «시간표» → «진호 시간표»(앱 로고). 그 안에 수업진도 대시보드 화면 셋(시간표계획·날짜별·진도표)을 웹앱 그대로 얹음(BrowserView) / v1.94.0 열쇠·설정 백업 — 파일 하나로 내보내기/가져오기, 암호(AES-256-GCM)로 잠금 / v1.93.0 오른쪽 단을 틱틱만으로 (내 할 일 화면 뺌 — 겹쳐서. 적어 둔 것은 상태 파일에 남아 있음) + 틱틱 로고 표시 / v1.92.1 틱틱 할 일이 스물몇 개씩 쏟아지던 것 — 기간(오늘·이번주·마감없음·전체)과 목록으로 거르기, 기본은 이번주 / v1.92.0 틱틱(TickTick) 연동 — 업무관리 오른쪽 단에서 «내 할 일»과 갈아 끼움. 핸드폰과 서로 반영 / v1.91.0 테마 둘 추가 — 한옥(한지 바탕·나무빛)·다크UI(짙은 남색·파랑). 대비를 재서 흐린 색은 짙게 조정 / v1.90.1 내 할 일 체크박스·✕ 와 업무관리 완료로·오늘로 단추가 눌러도 안 먹던 문제 — 단추 배선 목록에 새 종류가 빠져 있었다 / v1.90.0 학급 시간표에도 바뀐 수업 반영 — 컴시간이 변경분을 교사 시간표에만 줘서 학급 쪽은 옛 시간표였다. 교사 쪽 변경을 학급 칸에 옮겨 붙인다(빠짐·들어옴·맞바꿈) / v1.89.5 주간업무 가로 들여쓰기를 번호 단계로 통일 — 원문 앞 공백이 줄마다 달라 «2.» 와 그 아래 «가.» 가 같은 자리에 놓이던 문제 / v1.89.4 주간업무 해석기를 고쳐도 저장해 둔 옛 해석본이 그대로 보이던 문제 — 해석기 판(PARSE_VER)이 다르면 자동으로 다시 받는다 / v1.89.3 주간업무에서 «1.» 과 «가.» 가 같은 줄로 뭉개지던 문제 — 제 번호를 단 줄엔 목록 점을 안 붙인다 / v1.89.2 업무관리에서 «완료» 단추가 상태처럼 보이던 문제 — 모든 줄에 상태 배지(시작 전·진행 중) 표시, 단추는 «…로» 어미 외곽선으로 / v1.89.1 업무관리 2단 사이 칸막이를 끌어 너비 조절(시트처럼)·두 번 누르면 처음대로·너비 기억 / v1.89.0 업무관리 2단 — 오른쪽에 내가 쓰는 「내 할 일」(이 PC 저장·노션 무관). 완료는 접히고 복원·일괄삭제 가능 / v1.88.2 업무관리 탭을 눌러도 딴 탭으로 튕기던 문제(main VIEWS 에 task 누락) + 넓게 보기 차림표에도 업무관리 추가 / v1.88.1 v1.88.0 이 아예 안 켜지던 문제 — 새 파일 notion.js 가 빌드 목록에 빠졌었다(두 갈래 다 추가) / v1.88.0 업무관리 탭 신규(노션 PROJECTS·TASKS 연동 — 지비스 전용, 시간표와 주간업무 사이). 오늘·이번주/프로젝트별/마감없음 3갈래·완료·오늘로·진행중·추가 / v1.87.0 바뀐 수업(보강·교체)이 null-null 로 나오던 문제 — 컴시간이 «>11302» 같은 문자열로 주는 것을 못 읽었다. 파싱 보강 + 바뀐 칸 노란 테두리 표시 / v1.86.0 컴시간 자동 갱신(켤 때 1회 + 6시간마다) + 컴시간 탭 ⟳ 새로고침 버튼 / v1.85.0 숨은 창 로그인·캐시 메모리 전용(?widgetworker=1 — 격리로 손상되는 IndexedDB 회피, 로그인 몇 초 만에 풀리던 문제 뿌리 해결) + 켤 때 손상 창고·재기동 예약작업 찌꺼기 청소 / v1.84.0 구글 클라이언트를 파이어베이스와 같은 프로젝트 것으로 교체(audience 불일치 해소) + 판올림 재기동 wscript 숨김 실행(검은 콘솔 창 제거)
// 지비스 / 혜원 데스크 — 바탕화면에 항상 떠 있는 작은 카드 (한 벌의 코드에서 두 갈래로 빌드한다)
...
const { request } = require('./httpx.js');   // 학년부 일지 쓰기 다리를 부를 때
...
/* ★ 2026-09-11 확정 — 두 갈래 모두 여섯. 기본은 슬레이트 라이트.
     빠진 것(라벤더·한옥·다크UI·페르시안·슬레이트)을 쓰던 사람은 기본으로 간다.
     고스트는 CSS 로는 «테마 없음»('') 이지만, 저장은 'ghost' 로 한다 —
     '' 로 두면 «한 번도 안 고른 사람»과 갈리지 않아 기본(슬레이트 라이트)을 줄 수 없다. */
const THEMES = ['ghost', 'black', 'slatelight', 'chillwhite', 'night', 'gold'];
const THEME_DEFAULT = 'slatelight';
function themeList() { return THEMES; }
function getTheme() {
  const t = String(loadState().theme || '');
  /* 고른 적 없음·빠진 테마 → 기본. 고스트는 화면에 «테마 없음» 으로 준다 */
  const 고른것 = themeList().includes(t) ? t : THEME_DEFAULT;
  return 고른것 === 'ghost' ? '' : 고른것;
}
...
function getFontScale() {
  ...
  /* ★ 여기 없는 이름은 저장이 통째로 무시된다 — usage·task·tt 가 빠져 있었다(2026-09-04) */
  const out = { work: 1, comci: 1, cal: 1, meal: 1, rec: 1, home: 1, note: 1, link: 1,
    grid: 1, office: 1, usage: 1, task: 1, tt: 1, att: 1 };
...
/* 사용량 큰 상자를 펼쳐 두었나 — 지비스 제목 줄 스위치가 여닫는다 */
function getUsagePanel() { return !!loadState().usagePanel; }
...
/* ── 일지에 «쓰기» — GAS 다리 ─────────────────────────────
   ★ 왜 다리인가: 지비스의 구글 권한은 drive.file(이 앱이 만든 파일만) 이라
...
function writeShared(patch) {
  ...
  try {
    ...
    fs.writeFileSync(p, JSON.stringify(Object.assign(cur, patch), null, 2));
    return true;
  } catch (e) { debugLog('함께 쓰는 설정 저장 실패: ' + ((e && e.message) || e)); return false; }
}
...
function getGradeWrite() {
  ...
  const key = String(v.key || s.key || GRADE_WRITE_DEFAULT.key || "");
  return { url: url, key: key, from: v.key ? "pc" : (s.key ? "onedrive" : "") };
}
/* 다리를 한 번 부른다. ★ 구글이 웹앱 POST 를 다른 주소로 넘길 때 가끔 첫 번을
   놓친다(시험 중 실제로 겪었다). 그래서 한 번은 조용히 다시 보낸다. */
async function gradeBridge(몸) {
  const w = getGradeWrite();
  if (!w.url) throw new Error("일지 쓰기 주소가 없습니다 (설정 → 학사일정)");
  const 보냄 = Object.assign({ key: w.key }, 몸 || {});
  return bridgePost(w.url, 보냄, "다리가 제대로 답하지 않았습니다");
}
/* 다리(GAS)에 POST — ★ 구글은 결과를 googleusercontent 로 한 번 넘겨 돌려주는데 이 길이 가끔 끊긴다(2026-09-15 LG 혜원이지).
     ① 넘긴 곳이 404 HTML  ② 요청이 GET 으로 바뀌어 doGet 이 «알 수 없는 api: (없음)» 으로 답함 — 둘 다 «잠깐 끊김» 으로 보고
     1초·2초 쉬며 모두 세 번까지 다시 보낸다. (쓰기 요청은 nonce 로 다리가 두 번 넣지 않는다) */
async function bridgePost(url, 보냄, 실패말) {
  /* ★ v2.0.3 — 결과는 «한 번 열면 사라지는» 주소로 오고, 학교 망 보안 장비가 먼저 열어 가면 앱엔 404 가 남는다(LG 실측).
       그러면 다시 보내는 수밖에 없으니, 요청마다 번호(req)를 붙여 다리가 같은 번호의 쓰기는 처음 결과로 되돌려주게 한다
       (다리 1.5.0). 쉬는 틈은 1초·2.5초·5초 — 몰려서 나는 끊김을 넘기게 */
  보냄 = Object.assign({ req: Date.now().toString(36) + Math.random().toString(36).slice(2, 10) }, 보냄);
  const 쉼 = [0, 1000, 2500, 5000];
  let 마지막 = "";
  for (let i = 0; i < 쉼.length; i++) {
    if (쉼[i]) await new Promise((r) => setTimeout(r, 쉼[i]));
    let r;
    try {
      r = await request({ method: "POST", url, contentType: "application/json", body: JSON.stringify(보냄), timeout: 60000 });
    } catch (e) { 마지막 = (e && e.message) || String(e); continue; }
    let j = null;
    try { j = JSON.parse(r.text); } catch (e) { /* 글자 그대로 */ }
    const 끊김 = j && j.ok === false && /^알 수 없는 api/.test(String(j.msg || ""));
    if (j && j.ok !== undefined && !끊김) return j;
    마지막 = 끊김 ? "요청이 빈 채로 도착했습니다" : (r.text || "").slice(0, 120);
    debugLog("다리 — 잠깐 끊김, 다시 보냄 (" + (i + 1) + "/" + 쉼.length + "): " + 마지막.slice(0, 60));
  }
  throw new Error(실패말 + " — " + 마지막);
}

/* ── 담임 출결 ─────────────────────────────────────────────
   3학년 출결목록 시트에 다리(GAS)로 쓴다 — 시트 주소는 다리(비공개 저장소)만 안다.
   ★ 이 저장소는 공개라, 학생 이름·결석 사유가 보이는 시트 주소를 여기 두면 안 된다.
   여기서 정하는 것은 «우리 반 탭 이름» 과 «교사확인 칸에 적을 이름» 둘뿐이다. */
const ATT_DEFAULT = { cls: '3-2', teacher: '김진호' };
function getAttCfg() {
  const v = loadState().att || {};
  const s = readShared().att || {};
  return {
    cls: String(v.cls || s.cls || ATT_DEFAULT.cls),
    teacher: String(v.teacher || s.teacher || ATT_DEFAULT.teacher)
  };
}

/* 앱에만 두는 출결 — 시트 유형 목록에 없는 «미인정» 을 적어 두는 곳(2026-09-11 지시).
   시트에는 절대 쓰지 않는다. 두 PC 가 같이 보도록 OneDrive 에도 둔다(있으면 그쪽이 먼저). */
function getAttLocal() {
  const s = readShared().attLocal;
  const v = loadState().attLocal;
  const list = Array.isArray(s) ? s : (Array.isArray(v) ? v : []);
  return list.filter((o) => o && o.key);
}
function setAttLocal(list) {
  saveState({ attLocal: list });
  writeShared({ attLocal: list });
}

/* 신청사유 «내 예시 문구» — 누르면 사유 칸에 바로 들어가는 칩. 한 번도 안 정했으면 null(앱이 자주 쓴 사유를 보여 줌) */
function getAttPhrases() {
  const s = readShared(), v = loadState();
  const list = Array.isArray(s.attPhrases) ? s.attPhrases : (Array.isArray(v.attPhrases) ? v.attPhrases : null);
  return list ? list.map((x) => String(x || '').trim()).filter(Boolean).slice(0, 40) : null;
}
function getAttPhrasesAt() { return String(readShared().attPhrasesAt || loadState().attPhrasesAt || ''); }

/* 출결 «점검하기» — 서류·출석부·NEIS 체크(2026-09-12). ★앱에서만 — 시트엔 절대 안 쓴다.
   { '<학번|이름|일자|유형>': { doc, att, neis } }. 두 PC 가 같이 보도록 OneDrive 에도(있으면 그쪽이 먼저) */
function getAttChk() {
  const s = readShared().attChk, v = loadState().attChk;
  return (s && typeof s === 'object') ? s : ((v && typeof v === 'object') ? v : {});
}
function getAttChkAt() { return String(readShared().attChkAt || loadState().attChkAt || ''); }
...
/* 업무관리 2단의 오른쪽 너비(px).
   ★ 예전엔 180~620 을 «벗어나면 300 으로 되돌려» 넓게 잡아 둔 것이 통째로 날아갔다.
     이제는 자르기만 한다(되돌리지 않는다). 위 끝은 아주 넓은 화면까지 넉넉히 둔다. */
const SPLIT_MIN = 180, SPLIT_MAX = 2400;
...
function sendToWidget() {
  const payload = {
    ...
    dashVer: HAS_TT ? dashVer : null,
    theme: getTheme(),
    font: getFont(),
    fontScale: getFontScale(),
    todayEvent: todayEventText(),
    notes: notesToShow(),
    rec: Object.assign(recordsmain.recState(), { cls: String(loadState().recCls || ''), show: String(loadState().recShow || 'both') }),
    /* 그리는 쪽 안내 글이 «크롬» 이라고 못박지 않도록, 실제로 열리는 이름을 보낸다 */
    browserLabel: browserName(),
    flavor: FLAVOR,
    appName: APP_NAME,
    usage: { show: getUsageShow(), on: getUsageOn(), style: getUsageStyle(), panel: getUsagePanel(),
...
/* AI 사용량 — 트레이(작업표시줄) 풍선 도움말에 한 줄씩 붙인다.
   ★ 켠 것만(설정 → 사용량), 5시간 창 기준(없으면 주간). 지비스·혜원이지 둘 다 같다. */
function usageTipLine() {
  const on = getUsageOn();
  if (!on.length) return '';
...
function updateTrayTooltip() {
  if (!tray) return;
  let base;
  if (!lastData || lastData.needLogin) base = `${APP_NAME} — 로그인이 필요합니다`;
  else {
    const ls = lastData.lessons || [];
    if (!ls.length) base = `${APP_NAME} — 오늘(${lastData.dow}) 수업 없음`;
    else {
      const lines = ls.map((l) => `${l.period}교시 ${l.cls} ${l.unit}${l.n ? ' ' + l.n + '차시' : ''}`);
      base = `오늘 수업 ${ls.length}개\n` + lines.join('\n');
    }
  }
  const usage = usageTipLine();
  tray.setToolTip(usage ? base + '\n\n' + usage : base);
  /* B안 — 서비스마다 작은 아이콘을 알림 영역에 나란히(지비스만) */
  if (HAS_TT) usagetray.reconcile(getUsageOn(), aiusage.snapshot());
}
...
/* ── 얹은 수업진도 웹앱의 판 ─────────────────────────────────
   ★ 웹앱 제 머리(버전 배지)는 우리가 감춰 놓았으므로, 배포된 js/server.js 에서
     APP_VERSION·DEPLOYED_AT 을 그대로 읽는다. 탭을 안 열어도 보이고, 창을 띄우지
     않으니 가볍다. 여섯 시간에 한 번만 새로 읽는다. */
let dashVer = { ver: '', at: '', readAt: 0 };
let dashLoadedVer = '';        // 얹은 화면이 지금 띄우고 있는 판
let dashReloading = false;
/* ★ 이 웹앱은 PWA 라 서비스워커가 옛 판을 캐시에서 내준다 — 새로 배포해도 위젯만
   옛 화면인 까닭이 이것이다(2026-09-06). 얹은 창에서는 오프라인이 필요 없으니
   서비스워커와 캐시를 떼어 내고 늘 그물에서 받아 온다. */
async function dashDropSW() {
  if (!dashView || dashView.webContents.isDestroyed()) return;
  try {
    await dashView.webContents.executeJavaScript(
      '(async function(){ try {'
      ...
      + ' return true; } catch(e) { return false; } })()', true);
  } catch (e) { /* 아직 안 떴으면 다음 차례에 */ }
}
/* 얹은 화면이 띄운 판을 읽는다 — 웹앱이 제 머리에 적어 두는 값(감춰도 글자는 있다) */
async function dashReadLoadedVer() {
  if (!dashView || dashView.webContents.isDestroyed()) return '';
  try {
    const t = await dashView.webContents.executeJavaScript(
      '((document.getElementById("verBadge")||{}).textContent || "")', true);
    return (String(t).match(/v([0-9][0-9.]*)/) || [])[1] || '';
  } catch (e) { return ''; }
}
/* 새 판이 올라왔으면 곧바로 다시 불러온다 — 캐시를 건너뛰고 */
async function dashFreshen(why) {
  if (!dashView || dashView.webContents.isDestroyed() || dashReloading) return false;
  dashReloading = true;
  try {
    await dashDropSW();
    dashSkinNow = '';
    dashView.webContents.reloadIgnoringCache();
    debugLog('진호 시간표 다시 불러옴 — ' + (why || ''));
    return true;
  } finally { setTimeout(() => { dashReloading = false; }, 4000); }
}
const DASHVER_TTL = 6 * 3600 * 1000;
async function refreshDashVer(force) {
  if (!HAS_TT) return;
  if (!force && dashVer.ver && Date.now() - dashVer.readAt < DASHVER_TTL) return;
  try {
    const t = await fetchText(APP_URL + 'js/server.js?_w=' + Date.now(), { timeout: 12000 });
    const v = (String(t).match(/APP_VERSION\s*=\s*'([^']+)'/) || [])[1] || '';
    const at = (String(t).match(/DEPLOYED_AT\s*=\s*'([^']*)'/) || [])[1] || '';
    if (v) {
      const 바뀜 = v !== dashVer.ver;
      dashVer = { ver: v, at: at, readAt: Date.now() };
      if (바뀜) debugLog('수업진도 웹앱 판 — v' + v + (at ? ' · 배포 ' + at : ''));
      sendToWidget();
      /* ★ 얹어 둔 화면이 옛 판이면 그 자리에서 새로 받아 온다(즉각 반영) */
      if (dashView && !dashView.webContents.isDestroyed()) {
        const 띄운판 = await dashReadLoadedVer();
        if (띄운판) dashLoadedVer = 띄운판;
        if (dashLoadedVer && dashLoadedVer !== v) await dashFreshen('새 판 v' + v);
      }
    }
  } catch (e) { debugLog('수업진도 웹앱 판을 못 읽음 — ' + ((e && e.message) || e)); }
}
...
function dashEnsure() {
  ...
  dashView.webContents.on('did-finish-load', () => {
    ...
    refreshDashVer(true);                  // 새로 불렀으니 판도 다시 확인한다
    setTimeout(async () => {               // 서비스워커를 떼어 내고, 띄운 판을 적어 둔다
      await dashDropSW();
      dashLoadedVer = await dashReadLoadedVer();
    }, 1500);
...
ipcMain.on('dash-show', (e, screen, r, colors, zoom) => {
  ...
  v.setBounds(b);
  /* 얹은 화면 배율 — 아래 손잡이로 끈 값(창마다). 표·글자가 함께 커진다(v1.128.0) */
  const dz = Number(zoom) || 1;
  if (dz >= 0.8 && dz <= 3 && Math.abs(v.webContents.getZoomFactor() - dz) > 0.005) v.webContents.setZoomFactor(dz);
  dashSkin(colors);
...
ipcMain.handle('dash-reload', async () => {
  await refreshDashVer(true);              // 판부터 다시 읽고
  if (!dashView || dashView.webContents.isDestroyed()) return { ok: true, at: stampKST() };
  await dashFreshen('눌러서');
  return { ok: true, at: stampKST() };
});
...
ipcMain.handle('backup-export', async (_e, opt) => {
  ...
  try {
    ...
    const z = (n) => (n < 10 ? '0' : '') + n;
    const d = new Date();
    const name = '지비스-백업-' + d.getFullYear() + z(d.getMonth() + 1) + z(d.getDate()) + '.json';
...
ipcMain.handle('get-settings', () => ({
  ...
  ui: {
    ...
    /* ★ 설정 창에도 준다 — 안 주면 칸이 비어 보이고, 그대로 «저장» 을 누르면
         빈 값이 PC·OneDrive 의 열쇠를 지워 버린다(2026-09-11 발견) */
    gradeWrite: HAS_TT ? getGradeWrite() : null,
    att: HAS_TT ? getAttCfg() : null,
    rec: Object.assign(recordsmain.recState(), { cls: String(loadState().recCls || ''), show: String(loadState().recShow || 'both') }),
...
ipcMain.on('set-ui', (_e, v) => {
  ...
  if (v.taskSplit !== undefined) {
    ...
    if (isFinite(n)) saveState({ taskSplit: Math.min(SPLIT_MAX, Math.max(SPLIT_MIN, Math.round(n))) });
  }
  if (v.recClasses !== undefined) { saveState({ recClasses: v.recClasses || [] }); sendToWidget(); }
  /* 마지막에 고른 학급·보기 — 다시 켜도 그대로 */
  if (v.recCls !== undefined) { saveState({ recCls: String(v.recCls || '') }); sendToWidget(); }
  if (v.recShow !== undefined) { saveState({ recShow: String(v.recShow || 'both') }); sendToWidget(); }
  ...
  if (v.gradeWrite !== undefined && HAS_TT) {
    const cur = getGradeWrite();
    /* ★ 빈 값으로는 덮지 않는다 — 칸이 비어 보인 채 «저장» 을 눌러도 열쇠가 안 지워지게 */
    if (String(v.gradeWrite.url || "").trim()) cur.url = String(v.gradeWrite.url).trim();
    if (String(v.gradeWrite.key || "").trim()) cur.key = String(v.gradeWrite.key).trim();
    saveState({ gradeWrite: { url: cur.url, key: cur.key } });
    /* 다른 PC 도 같은 것을 쓰도록 OneDrive 에도 남긴다 */
    writeShared({ gradeWrite: { url: cur.url, key: cur.key } });
    debugLog('학년부 일지 쓰기 주소 변경 (OneDrive 에도 남김)');
    sendToWidget();
  }
  ...
  if (v.attColW !== undefined) {
    ...
    Object.keys(v.attColW || {}).forEach((k) => {
      ...
      if (/^[ck]\d$/.test(k) && n >= 20 && n <= 1200) o[k] = Math.round(n);   // k1~k3 = 점검 칸
    });
    saveState({ attColW: o });
  }
  ...
  if (v.attPhrases !== undefined) {
    ...
    saveState({ attPhrases: list, attPhrasesAt: at });
    writeShared({ attPhrases: list, attPhrasesAt: at });
    sendToWidget();
  }
  /* 오늘 주간표 크기 — 표 아래 손잡이로 끈 배율(창마다). 다시 보내지 않는다(화면이 이미 그 크기) */
  /* ★ 창은 제 몫(wid|easy)만 보낸다 — 두 창이 각자 옛 사본을 통째로 덮어쓰지 않게. 0 이면 처음대로 */
  if (v.grZoom !== undefined) {
  ...
  if (v.attChk !== undefined) {
    ...
    Object.keys(src).slice(0, 3000).forEach((k) => {
      const x = src[k] || {};
      const y = { doc: !!x.doc, att: !!x.att, neis: !!x.neis };
      if (y.doc || y.att || y.neis) o[String(k).slice(0, 200)] = y;
    });
    const at = String(v.attChkAt || '');
    saveState({ attChk: o, attChkAt: at });
    writeShared({ attChk: o, attChkAt: at });
  }
  /* 담임 출결 — 우리 반·교사확인 이름. 두 PC 가 같아야 하므로 OneDrive 에도 남긴다 */
  if (v.att !== undefined && HAS_TT) {
    const cur = getAttCfg();
    if (v.att.cls !== undefined) cur.cls = String(v.att.cls || '').trim();
    if (v.att.teacher !== undefined) cur.teacher = String(v.att.teacher || '').trim();
    const 값 = { cls: cur.cls, teacher: cur.teacher };
    saveState({ att: 값 });
    writeShared({ att: 값 });
    debugLog('출결 설정 변경 — ' + 값.cls + ' · ' + 값.teacher);
    sendToWidget();
  }
  ...
  if (v.usagePanel !== undefined) { saveState({ usagePanel: !!v.usagePanel }); sendToWidget(); }
...
function createTray() {
  ...
  if (HAS_TT) {
    ...
    usagetray.setOpener((key, u) => {
      if (u && u.needsLogin) { aiusage.openLogin(key); return; }
      checkForUpdates();
      if (easyWin && !easyWin.isDestroyed()) { easyWin.show(); easyWin.focus(); return; }
      if (!widgetWin || widgetWin.isDestroyed()) { createWidgetWindow(); return; }
...
/* 일지에 한 건 넣는다. 넣고 나면 그 학년 일지를 다시 받아 화면이 곧바로 맞게 한다. */
ipcMain.handle('grade-add', async (_e, p) => {
  if (!HAS_TT) return { ok: false, msg: "학년부 일지 쓰기는 지비스에서만 됩니다" };
  const o = p || {};
  try {
    const r = await gradeBridge({
      grade: Number(o.grade) || 3, date: String(o.date || ""),
      cat: String(o.cat || ""), title: String(o.title || ""),
      detail: String(o.detail || "")
    });
    if (r.ok) {
      debugLog("학년부 일지 — " + o.grade + "학년 " + o.date + " 넣음 (" + r.how + ")");
      try { await refreshGradePlan(Number(o.grade) || 3); } catch (e) { /* 화면은 곧 다시 받는다 */ }
    }
    return r;
  } catch (e) {
...
/* 방금 넣은 것을 되돌린다 — 그 줄의 내용이 넣은 그것과 같을 때만 손댄다 */
ipcMain.handle('grade-undo', async (_e, p) => {
  if (!HAS_TT) return { ok: false, msg: "학년부 일지 쓰기는 지비스에서만 됩니다" };
  const o = p || {};
  try {
    const r = await gradeBridge({
      op: "undo", grade: Number(o.grade) || 3,
      row: Number(o.row) || 0, expect: String(o.expect || "")
    });
    if (r.ok) {
...
/* 시트에 실제로 쓰이고 있는 «구분» 목록 — 칩으로 보여 준다 */
ipcMain.handle('grade-cats', async (_e, grade) => {
  if (!HAS_TT) return { ok: false, msg: "학년부 일지 쓰기는 지비스에서만 됩니다" };
  const w = getGradeWrite();
  ...
  try {
    const r = await request({
      method: "GET",
      url: w.url + "?api=cats&grade=" + (Number(grade) || 3)
        + "&key=" + encodeURIComponent(w.key),
      timeout: 30000
    });
...
async function staffCall(몸) {
  ...
  const j = await bridgePost(s.url, 보냄, '출결 연결이 제대로 답하지 않았습니다');   // 끊기면 세 번까지 다시
...
ipcMain.handle('att-local-edit', (_e, p) => {
  ...
  const it = list.filter((x) => x.key === String(o.key))[0];
  if (!it) return { ok: false, msg: '그 줄이 없습니다' };
  Object.assign(it, attFields(o), { at: new Date().toISOString() });
  setAttLocal(list);
...
ipcMain.handle('att-local-add', (_e, p) => {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(o.date || ''))) return { ok: false, msg: '일자를 골라 주세요' };
  const list = getAttLocal();
  const key = Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
  list.push({ key, id: String(o.id), name: String(o.name || ''), date: String(o.date),
    time: String(o.time || ''), end: String(o.end || ''), type: String(o.type),
    reason: String(o.reason || '').trim(), at: new Date().toISOString() });
  setAttLocal(list);
  debugLog('출결(앱에만) — ' + o.id + ' ' + o.date + ' ' + o.type);
...
const PRINT_CSS = `
  ...
  * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }   /* 배경색 늘 찍기(2.0.1) */
  body {
...
const PAPER_BAR_CSS = `
  ...
  .pbar #pmsg { font-weight: 600; font-size: 12px; opacity: .92; }
  @media print { .pbar { display: none !important; } }
`;
/* ★ 다리(paperload.js)가 없어도 단추가 죽지 않게 — 브라우저 기본 인쇄·닫기로 물러선다.
     (v1.125.0 까지 paperload.js 가 빌드 목록에 빠져 세 단추가 모두 먹통이었다 — 2026-09-12)
     PDF 는 기본 인쇄 창의 «PDF로 저장» 으로 대신한다. */
const PAPER_BAR_HTML = '<div class="pbar">'
  + '<b>미리보기</b><span id="pmsg">실제로 나올 종이 그대로입니다</span><span class="sp"></span>'
  + '<button class="no" onclick="window.paper ? paper.close() : window.close()">닫기</button>'
  + '<button class="no" onclick="window.paper ? paper.pdf() : window.print()">PDF 저장</button>'
  + '<button class="go" onclick="window.paper ? paper.print() : window.print()">🖨 인쇄</button></div>';

function printPaper(p, owner) {
  /* ★ 이미 열려 있으면 새로 만들지 않고 앞으로 가져온다 — 뒤에 숨은 창을 또 부르는 일이 없게 */
  if (paperWin && !paperWin.isDestroyed()) {
    paperFront(paperWin);
    return Promise.resolve('front');
  }
  return new Promise((resolve) => {
    // ★ 이제 «보여 주는» 창이다 — 눈으로 보고 나서 인쇄를 누른다
    /* ★ 부른 창(위젯·넓은 창)의 «딸린 창» 으로 띄운다. 딸린 창은 주인이 «항상 위»·전체화면이어도
         늘 그 위에 서고, 인쇄 대화상자도 이 창에 딸려 함께 위에 뜬다.
         (v1.124.0 까지는 따로 선 창이라 위젯 뒤로 숨어 «인쇄가 안 된다» 로 보였다 — 2026-09-11) */
    const parent = owner && !owner.isDestroyed() ? owner : null;
    const win = new BrowserWindow({
      width: 900, height: 1000, show: false,
      parent: parent || undefined, modal: false,
    ...
    const head = '<!doctype html><html lang="ko"><head><meta charset="utf-8">'
      ...
      + '<style>' + PRINT_CSS + PAPER_BAR_CSS + (p.css || '') + '</style></head><body>';
    /* ★ plain — 시트를 그대로 옮긴 종이(출결). 제목·앱 이름·뽑은 날처럼
         시트 인쇄에 없는 것은 하나도 넣지 않는다(2026-09-11 지시) */
    const paper = (p.plain ? '' : '<div class="head"><h1>' + escHtml(p.range || '주간업무') + '</h1>'
        + '<div class="sub">' + escHtml(p.doc || '') + ' · ' + escHtml(APP_NAME)
        + ' · 뽑은 날 ' + stampNow() + '</div></div>')
      + (p.body || '');
    const html = head + PAPER_BAR_HTML + paper + '</body></html>';
    /* 브라우저로 넘길 때 쓸 종이 — 미리보기 줄은 빼고, 열리자마자 인쇄 창을 띄운다 */
    /* ★ 브라우저 인쇄는 기본으로 «머리글·바닥글»(날짜·제목·파일 주소·쪽 번호)을 붙인다(2026-09-15 선생님 화면).
         @page 여백을 0 으로 두면 붙일 자리가 없어 빠진다 — 대신 여백은 종이 안에 만든다:
         위·아래는 쪽마다 되풀이되는 thead·tfoot 빈 줄, 좌우는 칸 안쪽 여백. 모양은 시트 인쇄 그대로 */
    const mg = Array.isArray(p.margin) && p.margin.length === 3 ? p.margin.map(Number) : [12, 12, 14];
    const 여백css = '<style>@page{size:A4;margin:0 !important}html,body{margin:0 !important;padding:0 !important}'
      + '*{-webkit-print-color-adjust:exact;print-color-adjust:exact}'
      + '.pgw{width:100%;border-collapse:collapse;border:0}'
      + '.pgw>thead>tr>td{height:' + mg[0] + 'mm;padding:0;border:0}'
      + '.pgw>tfoot>tr>td{height:' + mg[2] + 'mm;padding:0;border:0}'
      + '.pgw>tbody>tr>td{padding:0 ' + mg[1] + 'mm;border:0;vertical-align:top}</style>';
    paperDoc = { title: String(p.title || p.range || '출력'),
    ...
    // 창이 닫히면(엑스를 눌러도) 기다리던 쪽을 풀어 준다 — 내려 두었던 위젯도 «항상 위» 로 되돌린다
    win.once('closed', () => { paperWin = null; paperRestoreWidget(); resolve(true); });
...
/* ★ 위젯은 «항상 위» 중 가장 높은 층(screen-saver)에 떠 있어서, 보통 창인 미리보기가
     그 뒤에 열렸다 — 넓게 펴 둔 위젯에 완전히 가려져 «인쇄 단추가 안 눌린다» 로 보였다(2026-09-11).
     미리보기가 떠 있는 동안만 위젯의 «항상 위» 를 내려 두고, 미리보기를 맨 앞으로 올린다.
     (미리보기를 위젯 층으로 올리면 윈도우 인쇄 대화상자가 그 뒤로 숨을 수 있어 이렇게 한다) */
let paperLowered = false;
function paperFront(win) {
  if (!win || win.isDestroyed()) return;
  if (widgetWin && !widgetWin.isDestroyed() && widgetWin.isAlwaysOnTop()) {
...
function paperRestoreWidget() {
  if (!paperLowered) return;
  paperLowered = false;
  if (widgetWin && !widgetWin.isDestroyed() && getAlwaysOnTop()) widgetWin.setAlwaysOnTop(true, 'screen-saver');
}
function escHtml(t) {
...
/* 미리보기 창에서 오는 세 가지 — 인쇄 · PDF 저장 · 닫기 */
let paperWin = null;
/* 미리보기 줄에 한 줄 알림 — 조용히 실패하지 않게 */
function paperSay(msg) {
  if (!paperWin || paperWin.isDestroyed()) return;
  paperWin.webContents.executeJavaScript('(function(){var e=document.getElementById("pmsg");if(e)e.textContent='
    + JSON.stringify(String(msg)) + ';})()').catch(() => {});
}
/* ── 브라우저로 인쇄 (v1.129.0) ─────────────────────────────────
   ★ 학교 PC 에서 Electron 의 윈도우 인쇄 창이 아예 안 뜬다(2026-09-14 LG 실측 — 대화상자도 콜백도 없음,
     프린터 지정 인쇄는 15초 뒤 «Invalid printer settings»). 출력물 보안(iCertPrintClient) 등이 낯선 exe 를
     막는 것으로 보인다. 프린터 목록·PDF 굽기는 된다.
     → 종이를 임시 HTML 로 써서 «기본 브라우저» 로 열고, 열리자마자 브라우저의 인쇄 창을 띄운다.
     먼저 제 인쇄 창을 불러 보고 5초 안에 안 뜨면 브라우저로 넘긴다. 한 번 넘긴 PC 는 다음부터 곧장 브라우저로. */
let paperDoc = null;
const PRINT_DIR = () => path.join(app.getPath('temp'), 'jivis-print');
function paperCleanTemp(maxAgeMs) {
  try {
    const d = PRINT_DIR();
    fs.readdirSync(d).forEach((f) => {
      const fp = path.join(d, f);
      try { if (Date.now() - fs.statSync(fp).mtimeMs > maxAgeMs) fs.unlinkSync(fp); } catch (e) { /* 쓰는 중 */ }
    });
  } catch (e) { /* 폴더 없음 */ }
}
async function paperToBrowser(why) {
  ...
  try {
    ...
    paperCleanTemp(10 * 60 * 1000);                 // 학생 이름이 든 종이다 — 오래 남기지 않는다
    const name = paperDoc.title.replace(/[\\/:*?"<>|]+/g, ' ').trim().slice(0, 60) || '출력';
    const fp = path.join(d, name + ' ' + Date.now() + '.html');
    fs.writeFileSync(fp, paperDoc.html, 'utf8');
    const err = await shell.openPath(fp);          // .html 을 여는 기본 프로그램 = 브라우저
...
ipcMain.on('paper-print', () => {
  if (!paperWin || paperWin.isDestroyed()) return;
  /* 이 PC 에서 제 인쇄 창이 안 떴던 적이 있으면 곧장 브라우저로 */
  if (loadState().printVia === 'browser') { paperToBrowser('이 PC 는 브라우저 인쇄'); return; }
  paperSay('인쇄 창을 여는 중…');
  let settled = false;
  const giveUp = setTimeout(() => {               // 5초 안에 대답도 창도 없으면 막힌 것으로 본다
    if (settled) return;
    settled = true;
    saveState({ printVia: 'browser' });
    paperToBrowser('인쇄 창이 5초 안에 안 뜸');
  }, 5000);
...
/* PDF 로 저장 — 프린터 창이 말썽일 때 쓰는 길. 저장한 곳을 탐색기로 보여 준다 */
ipcMain.on('paper-pdf', async () => {
  if (!paperWin || paperWin.isDestroyed()) return;
  const win = paperWin;
  try {
    const title = String(win.getTitle() || '출력').replace(/[\\/:*?"<>|]+/g, ' ').trim() || '출력';
    const r = await dialog.showSaveDialog(win, {
      title: 'PDF 로 저장',
      defaultPath: path.join(app.getPath('downloads'), title + '.pdf'),
      filters: [{ name: 'PDF', extensions: ['pdf'] }]
    });
    if (r.canceled || !r.filePath) { paperSay('PDF 저장을 취소했습니다'); return; }
    const buf = await win.webContents.printToPDF({ printBackground: true, preferCSSPageSize: true, pageSize: 'A4' });
    fs.writeFileSync(r.filePath, buf);
    paperSay('✅ PDF 저장됨 · ' + stampNow() + ' · ' + path.basename(r.filePath));
    shell.showItemInFolder(r.filePath);
    debugLog('PDF 저장 — ' + r.filePath);
  } catch (e) {
    paperSay('★ PDF 저장이 안 됐습니다: ' + ((e && e.message) || e));
    debugLog('PDF 저장 실패: ' + ((e && e.message) || e));
  }
});
...
ipcMain.handle('work-print', async (e, p) => {
  if (!p || !p.body) return false;
  return printPaper(p, BrowserWindow.fromWebContents(e.sender));
});
...
if (!gotLock) {
  ...
  try {
    fs.appendFileSync(path.join(app.getPath('userData'), 'debug.log'),
      '[' + new Date().toISOString() + '] 이미 떠 있는 지비스가 있어 이 판은 물러납니다'
      + ' (v' + app.getVersion() + ')' + String.fromCharCode(10));
  } catch (e) { /* 무시 */ }
  app.quit();
} else {
  ...
  app.whenReady().then(() => {
    ...
    paperCleanTemp(0);   // 지난번 브라우저 인쇄로 남은 임시 종이(학생 이름) 치우기
    ...
    if (HAS_TT) {
      ...
      /* 진호 시간표(수업진도 웹앱) 판 — 켤 때 한 번 + 여섯 시간마다 */
      scheduleTask('dashver', '수업진도 판', 20 * 1000, () => refreshDashVer(true));
      setInterval(() => refreshDashVer(true), 6 * 60 * 60 * 1000);

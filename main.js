...
function printPaper(p, owner) {
  ...
  return new Promise((resolve) => {
    ...
    const html = '<!doctype html><html lang="ko"><head><meta charset="utf-8">'
      ...
      + (p.plain ? '' : '<div class="head"><h1>' + escHtml(p.range || '주간업무') + '</h1>'
...
ipcMain.on('paper-print', () => {
  ...
  paperWin.webContents.print({ silent: false, printBackground: true }, (ok, why) => {
    ...
    paperSay('★ 인쇄가 안 됐습니다(' + why + ') — «PDF 저장» 으로 저장해 인쇄해 주세요');
    debugLog('인쇄 실패: ' + why);
  });
});

...
async function refreshTick(why) {
  ...
  try {
    /* ★ 기본함은 목록 목록에 안 나온다 — 처음 한 번 아이디를 알아내 적어 두고 늘 함께 읽는다 */
    let inbox = String((loadState().tick || {}).inboxId || '');
    if (!inbox) {
      try {
        inbox = await ticktick.findInbox(getTickKey());
        saveTick({ inboxId: inbox });
        debugLog('틱틱 기본함 찾음 — ' + inbox);
      } catch (e) { debugLog('틱틱 기본함 못 찾음 — ' + ((e && e.message) || e)); }
    }
    const r = await ticktick.loadAll(getTickKey(), inbox);

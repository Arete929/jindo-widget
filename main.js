...
const GRADE_WRITE_DEFAULT = {
  url: "https://script.google.com/macros/s/"
    + "AKfycbwkk8ZfyrvNSoKJ41VC9ZD66FsJx4Ivd0n0iv_dBrtDm1aSkHfP1cxP1iFRgfS6bexYdQ/exec",
  key: "hyewon-gradeplan"
};
function getGradeWrite() {
  const v = loadState().gradeWrite || {};
  return {
    url: v.url === undefined ? GRADE_WRITE_DEFAULT.url : String(v.url || ""),
    key: v.key === undefined ? GRADE_WRITE_DEFAULT.key : String(v.key || "")
  };
}
...
function sendToWidget() {
  const payload = {
    ...
    gradeWrite: getGradeWrite(),                 // 일지 쓰기 다리 (지비스만)
...
ipcMain.on('set-ui', (_e, v) => {
  ...
  if (v.gradeWrite !== undefined) {
    ...
    saveState({ gradeWrite: cur });
    debugLog('학년부 일지 쓰기 주소 변경');
    sendToWidget();
  }
